...
import { getFlash, type PagedResult } from "@/lib/sanity/sanity-api";
...
export async function fetchMoreFlash(
  start: number,
  end: number
  ...
  try {
    ...
    const data = await getFlash(params.start, params.end);
    ...
    if (!isKnownError(error)) {
      errorLogger.log(
        createAppError(
          ErrorType.UNKNOWN,
          "Unexpected error in fetchMoreFlash",
          error,
          { start, end }
        )
      );
    ...
    return {
      success: false,
      error: "Failed to load flash. Please try again.",
    };
  }
}
